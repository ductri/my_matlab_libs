function [MSE, A_est_order, M]= mse_measure_norm(A_est,A_true)
% =====================================
% Input:
...
%======================================
[m,k] = size(A_true);
A_est = A_est + 1e-20*ones(size(A_est));
A_true = A_true + 1e-20*ones(size(A_true));
for kk=1:k
    A_true(:,kk)=A_true(:,kk)/norm(A_true(:,kk),2);
    A_est(:,kk)=A_est(:,kk)/norm(A_est(:,kk),2);
end
A_true = normalize(A_true, 'norm');

[A_est_order, M]   = nearest(A_est,A_true);
