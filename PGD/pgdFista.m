function [x_k, tracking] = pgdFista(g_fn, p_fn, step_size, init_point, ops) 

    if isfield(ops, 'maxIters'), maxIters = ops.maxIters; else maxIters = 500; end;
    ...
    if isfield(ops, 'fFn'), fFn = ops.fFn; end;
    ...
    tracking = struct;
    tracking.obj = 100*ones(maxIters, 1);
    tracking.norm_g2 = 100*ones(maxIters, 1);
    tracking.time = 100*ones(maxIters, 1);
    tracking.dis2 = 100*ones(maxIters, 1);
    tracking.Stepsize = 100*ones(maxIters, 1);
    ...
    for i=1:maxIters
        ...
        duration = duration + toc(step_tic);
        if DEBUG, tracking.obj(i) = fFn(x_k); end;
