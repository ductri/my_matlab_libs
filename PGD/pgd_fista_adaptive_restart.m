function [x_k, tracking] = pgd_fista_adaptive_restart(g_fn, p_fn, step_size, init_point, ops) 
    ...
    if VERBOSE, fprintf('Running pgd_fista_adaptive_restart with DEBUG=%d ...\n', DEBUG); end;

    tracking = struct;
    tracking.obj = 100*ones(max_iters, 1);
    tracking.norm_g = 100*ones(max_iters, 1);
    tracking.time = 100*ones(max_iters, 1);
    tracking.restart_interval = [0];
    tracking.restart_criteria = 100*ones(max_iters, 1);
    ...
    for i=1:max_iters
        ...
        restart_criteria = g'*(x_k - x_km1);

        if DEBUG, tracking.restart_criteria(i) = restart_criteria; end;
        if  restart_criteria > 0
            % if VERBOSE, fprintf('Restarting ...\n'); end;
            x_km1 = x_k; y_k = x_km1; t_k = 1;
            duration = duration + toc(step_tic);

            if DEBUG, tracking.time(i) = duration; end;
            if DEBUG, tracking.restart_interval(end+1) = i - sum(tracking.restart_interval); end;
            duration = duration + toc(step_tic);
